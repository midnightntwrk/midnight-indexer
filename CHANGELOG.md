...
- *(chain-indexer)* Tolerate a runtime-upgrade enactment block in `get_contract_state`

  At a runtime-upgrade enactment block the MNSV protocol-version header digest lags the block's live
  runtime by one block, so the digest-selected subxt module is one version behind and
  `get_contract_state` failed with "the static Runtime API address used is not compatible with the
  live chain" whenever that block contained a contract action (mainnet block 1,774,491, spec
  22000 → 1000000, 2026-07-20). The call now tries the digest-selected module first, then falls back
  to the other module(s); subxt admits only the module matching the block's live metadata, so the
  fallback returns the correct state with no behaviour change for ordinary blocks. This consolidates
  the previously unreleased `4.3.3-hotfix` build (mainnet blue/green have been running it) onto 4.3.5.
