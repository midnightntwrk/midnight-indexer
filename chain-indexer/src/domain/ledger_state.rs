...
/// `MEMPOOL_TBLOCK_BUMP_MILLIS` off the parent block time, for a block built by the runtime with
/// the given protocol version; that is the runtime recorded in the block's MNSV digest, not the one
/// in its state, which is newer at a runtime-upgrade enactment block.
///
/// - 0.22 and 1.0 before `FIRST_UNSKEWED_NODE_1_0_SPEC_VERSION` serve the first transaction's
///   validity from the strict cache warmed during mempool ingress, i.e. verify it at the bumped
///   `tblock`.
/// - 1.0 from `FIRST_UNSKEWED_NODE_1_0_SPEC_VERSION` on (`Ledger8Bridge` version 2) verifies it
///   against the block's own time.
///
