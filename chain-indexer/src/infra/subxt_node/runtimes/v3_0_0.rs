...
pub async fn fetch_authorities(
    ...
    let authorities = block
        .storage()
        .entry(super::runtime_3_0_0::storage().aura().authorities())
...
pub fn decode_slot(mut slot: &[u8]) -> Result<u64, SubxtNodeError> {
    let slot = super::runtime_3_0_0::runtime_types::sp_consensus_slots::Slot::decode(&mut slot)
        .map(|x| x.0)?;
    Ok(slot)
}
...
pub async fn get_zswap_merkle_tree_root(
    ...
    let get_zswap_state_root = super::runtime_3_0_0::runtime_apis()
...
pub async fn get_ledger_state_root(
    ...
    let get_ledger_state_root = super::runtime_3_0_0::runtime_apis()
        .midnight_runtime_api()
...
pub async fn get_d_parameter(block: &OnlineClientAtBlock) -> Result<DParameter, SubxtNodeError> {
    let get_d_param = super::runtime_3_0_0::runtime_apis()
...
pub async fn fetch_genesis_cnight_registrations(
    ...
    let query = super::runtime_3_0_0::storage()
        .c_night_observation()
        .mapping();
    block
        .storage()
...
pub async fn get_terms_and_conditions(
    ...
    let get_tc = super::runtime_3_0_0::runtime_apis()
        .system_parameters_api()
