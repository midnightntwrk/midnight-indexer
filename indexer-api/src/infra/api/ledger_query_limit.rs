...
//! (`runtime/builder.rs`, `build_threaded_runtime`). At the cap the next core handoff is queued
//! with no thread to run it (`runtime/blocking/pool.rs`), so cores go dark one by one until the
//! runtime stops making progress and even `/live` stops answering. Nothing else in either binary is
//! a meaningful user of the blocking pool, so ledger walks are the only realistic way to reach the
//! cap — and unauthenticated queries such as `dustGenerationMerkleTreeUpdate` reach the walk with
//! no credentials, passing every existing limit (`max_complexity`, `limit_depth`, the
//! WebSocket-only subscription quota). Issue #595, "amber-heron".
//!
...
//! pool only caps how many can be resident at all. Every ledger-DB-touching resolver acquires one
//! permit *for the duration of its ledger work and no longer*: one-shot queries hold it across
//! their single walk, and subscriptions acquire per emitted item rather than once per stream, so a
...
/// pool, capped below `max_blocking_threads` and clamped to at least one.
///
...
/// serve them. In cloud, 25 connections give the arena 12.
///
/// In standalone the ledger DB is SQLite pinned to a single connection, so the share floors out at
/// one permit — walks already serialize on that connection, and admitting more would only pin
...
pub fn default_permits(
    ...
    let by_connections = arena_budget;
...
mod tests {
    ...
    #[test]
    fn permits_default_to_the_arena_share_of_the_pool() {
        // 25 connections give the arena 12, leaving the rest for other resolvers.
        assert_eq!(
    ...
    fn permits_never_zero_on_a_single_connection_pool() {
        ...
        // share must floor at one rather than deadlocking the resolvers at zero.
        assert_eq!(
