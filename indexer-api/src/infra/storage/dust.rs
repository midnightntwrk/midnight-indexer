...
impl DustStorage for Storage {
    ...
    //             let hex_len = prefix.as_ref().len() * 2;
    //             format!(
    //                 "substring(encode(nullifier, 'hex'), 1, {hex_len}) = encode(${param_num},
    // 'hex')"             )
    //         })
    //         .collect::<Vec<_>>();
    //
    ...
    //     let query = format!(
    //         indoc! {"
    //             SELECT COALESCE(MAX(b.height), $1) as highest_block, COUNT(DISTINCT t.id) as
    // matched_count             FROM dust_utxos du
    //             INNER JOIN transactions t ON t.id = du.spent_at_transaction_id
    //             INNER JOIN blocks b ON b.id = t.block_id
    //             WHERE du.nullifier IS NOT NULL
    //             AND ({})
    ...
    //             let hex_len = prefix.as_ref().len() * 2;
    //             format!(
    //                 "substring(encode(commitment, 'hex'), 1, {hex_len}) = encode(${param_num},
    // 'hex')"             )
    //         })
    //         .collect::<Vec<_>>();
    //
    ...
    //     let query = format!(
    //         indoc! {"
    //             SELECT COALESCE(MAX(merkle_index), $1) as highest_index, COUNT(*) as
    // commitment_count             FROM dust_utxos
    //             WHERE merkle_index >= $1
    //             AND ({})
    //         "},
    //         where_clause
    //     );
    //
    //     let mut progress_query = sqlx::query_as::<_, (i64, i64)>(&query).bind(start_index as
    // i64);
    //
    //     for prefix in &valid_prefixes {
    //         progress_query = progress_query.bind(prefix.as_ref());
    ...
    //     let query = format!(
    //         indoc! {"
    //             SELECT COALESCE(MAX(registered_at), 0) as latest_timestamp, COUNT(*) as
    // update_count             FROM cnight_registrations
    //             WHERE {}
    //         "},
    //         where_clause
    //     );
    ...
    //         match address.address_type {
    //             AddressType::CardanoStake | AddressType::Dust => {
