...
--------------------------------------------------------------------------------
ALTER TABLE dust_generation_info ADD COLUMN dust_epoch INTEGER NOT NULL DEFAULT 0;
...
CREATE INDEX dust_generation_info_owner_epoch_generation_index_idx
    ON dust_generation_info (owner, dust_epoch, generation_index);

-- Serves the dtime-update join, which matches on night_utxo_hash and must not
-- pick up the same UTXO's row from a dead epoch.
CREATE INDEX dust_generation_info_night_utxo_hash_epoch_idx
    ON dust_generation_info (night_utxo_hash, dust_epoch);
