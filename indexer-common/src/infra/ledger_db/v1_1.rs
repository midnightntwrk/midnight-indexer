...
mod sqlite_tests {
    ...
    /// concurrent references to one arena node (the ledger's delta-tracking `ChildRef` persists
    /// its child on creation and unpersists it on drop). Any value >= 3 exposes the read bug.
    const PERSISTS: u32 = 5;

    async fn setup_pool() -> Result<SqlitePool, Box<dyn StdError>> {
    ...
    /// which is what takes the chain-indexer down once the ledger drops a shared reference.
    #[tokio::test(flavor = "multi_thread")]
