...
fn insert_node<H>(key: ArenaHash<H>, object: OnDiskObject<H>)
where
    H: WellBehavedHasher,
{
    let mut value = Vec::with_capacity(object.serialized_size());
    SerializableV6::serialize(&object, &mut value).unwrap_or_panic("cannot serialize object");
    ...
    QueryBuilder::new(query)
        .push_values([(key, value)], |mut q, (k, v)| q.push_bind(k).push_bind(v))
        .build()
        .execute(todo!())
        .await;
