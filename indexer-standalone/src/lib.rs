...
/// Run the standalone indexer using its conventional configuration sources.
#[cfg(feature = "standalone")]
...
/// Run the indexer inside another process using an explicit configuration file.
///
/// The host owns process-wide logging and tracing. Indexer metrics retain their existing
/// configuration because they use a separate exporter and listener.
#[cfg(feature = "standalone")]
pub fn run_embedded(config_path: impl AsRef<std::path::Path>) -> anyhow::Result<()> {
    use anyhow::Context;
    use indexer_common::config::ConfigExt;

    let config = Config::load_from(config_path).context("load embedded indexer configuration")?;
    run_with_config(config, false)
}

#[cfg(feature = "standalone")]
fn run_with_config(config: Config, initialize_tracing: bool) -> anyhow::Result<()> {
    use anyhow::Context;
    use chain_indexer::{
    ...
    runtime.block_on(async {
        ...
        } else if tracing_config.enabled {
            warn!("embedded indexer tracing is disabled because the host owns global tracing");
        }
