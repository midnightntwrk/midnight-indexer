...
fn run() -> anyhow::Result<()> {
    ...
    // single writer, see `ledger_db::init` — so the default lands on one permit: walks
    // already serialize on that connection and admitting more would only pin blocking threads
    // waiting for it. Note this is the ledger DB's own pool, not the main storage pool, which
    // is wider.
