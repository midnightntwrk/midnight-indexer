...
async fn hardfork_8_to_9_crossing() -> anyhow::Result<()> {
    ...
    *harness.indexer.lock().expect("indexer mutex poisoned") = Some(start_indexer(
    ...
    // --- 4a. Open the idle progress subscription -----------------------------
    //
    // Opened now, so it spans the upgrade rather than being established after it. Skipped,
    // loudly, on a build without the field.
    ...
    let progress = if harness.progress_protocol_version_supported().await? {
        ...
        println!("[4a] watching progress updates for an address with no transactions");
        Some(watcher)
    } else {
        println!(
            "[4a] WARNING: this build does not serve UnshieldedTransactionsProgress.\
