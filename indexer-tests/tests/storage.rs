...
use chain_indexer::domain::{storage::Storage as _, BlockInfo};
use futures::{StreamExt, TryStreamExt};
use indexer_api::domain::{ContractAction, ContractAttributes, Storage as _};
use indexer_common::{
    cipher::make_cipher,
...
async fn run_tests(
    ...
    let mut transactions = block.transactions.into_iter();
    let transaction = transactions.next().unwrap();
    ...
    assert_matches!(
        transaction.contract_actions.as_slice(),
        [
            ContractAction {
    ...
    assert_eq!(transaction.contract_actions.len(), 3);
    assert_matches!(
        transaction.contract_actions.as_slice(),
        [
            ContractAction {
