...
pm18678-stop:
    ...
    docker stop postgres nats node 2>/dev/null || true
    docker rm postgres nats node 2>/dev/null || true
    # Clean up target/data for next session
...
pm18678-purge:
    ...
    if [[ $REPLY == "PURGE" ]]; then
        ...
        docker stop postgres nats node 2>/dev/null || true
        docker rm -v postgres nats node 2>/dev/null || true
