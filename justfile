...
coverage:
    ...
    tests=$(cargo test -p indexer-tests --test native_e2e --features {{feature}} --no-run --message-format=json | jq -r 'select(.profile.test == true and .target.name == "native_e2e") | .executable')
    # Then setup for coverage instrumentation and build the executables (spawned in the tests).
