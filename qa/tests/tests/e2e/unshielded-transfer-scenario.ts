...
async function expectProgressUpdate(
  ...
  const event = await retry(
    async () => {
      // A closed or silent socket will never deliver the event, so stop waiting
      // and say why. Without this the test burns its full five-minute budget and
      // then reports only "not found yet", which hides the real cause.
