...
/**
 * Subscribe to a wallet that will never transact and sit idle, reporting whether
 * the socket survived.
 *
 * The address is derived from a random seed, so it has no history and no
 * incoming transfers: after the initial empty train the indexer has nothing to
 * say, which is the condition under test.
 *
 ...
 * measuring nothing — which is exactly what the first version of this file did.
 */
async function surviveIdle(client: IndexerWsClient, windowMs: number): Promise<boolean> {
  // Derived locally rather than through `deriveAddresses`, which dispatches on
  // TX_BACKEND and would demand a toolkit container on the default backend. This
  // test needs nothing but a well-formed address nobody uses, and key derivation
  // is pure local computation.
  const network = env.getNetworkId().toLowerCase();
  const derived = deriveAllAddressesFromSeed(randomBytes(32).toString('hex'));
  const address = derived.nightExternal.bech32m[network];
  client.subscribeToUnshieldedTransactionEvents({ next: () => {} }, { address });
