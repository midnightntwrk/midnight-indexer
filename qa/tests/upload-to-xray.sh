...
if [ -z "$XRAY_CLIENT_ID" ]; then
    ...
    echo "Please set XRAY_CLIENT_ID environment variable before running this script"
    exit 1
fi

if [ -z "$XRAY_CLIENT_SECRET" ]; then
    echo "Error: XRAY_CLIENT_SECRET environment variable is not set"
    echo "Please set XRAY_CLIENT_SECRET environment variable before running this script"
    exit 1
fi
