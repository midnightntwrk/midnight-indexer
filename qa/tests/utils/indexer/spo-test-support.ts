...
// are clamped silently into 1..max, never rejected, so the probes below assert
// on bounds.
...
/**
 * The `0x` / `0X` prefixed and upper-case spellings of a hex identifier. The
 * pool-id and SPO-key resolvers normalise all of them to lowercase without a
 * prefix (query.rs `normalize_hex`), so every variant must resolve identically.
 */
export function hexCaseAndPrefixVariants(hex: string): string[] {
  const upper = hex.toUpperCase();
  return [`0x${hex}`, `0X${hex}`, upper, `0x${upper}`];
}
 ...
 *
 * currentEpochInfo is extrapolated to the chain's real current epoch while
 * spo-indexer backfills committee membership one epoch per poll cycle and has
 * been observed trailing by ~4 epochs on devnet. The window is scanned newest
 * first in concurrent chunks so the usual lag resolves in a single round trip
 * instead of one request per epoch.
 *
