...
- __XRAY_TEST_EXEC_KEY__: This is the test execution key (a Jira ID for the TestExecution type) and it's optional if the below is provided, in which case Xray will create a new one

- __XRAY_TEST_PLAN_KEY__: This is the test plan key (a Jira ID for the TestPlan type) and it's optional. If not provided, a default Test Plan Key will be used
...
- __summary__: This is the title of the XRay test case that appears on top of the Jira issue. This is created using [test suite name], [test case name] or just [test case name], in the unlikely event that there isn't a test suite defined 
