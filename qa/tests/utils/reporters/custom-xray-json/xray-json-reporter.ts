...
export default class XRayJsonReporter implements Reporter {
  ...
  onFinished(files: RunnerTestSuite[]) {
    ...
    if (!process.env.XRAY_COMPONENT) {
      console.debug(
        "WARNING: XRAY_COMPONENT env variable not set, 'unknown component' will be used",
      );
    }
    ...
    const xrayTests: XRayTest[] = testcases.map((test) => {
    ...
    const xrayReport: XRayReport = {
      info: {
        summary: `Test execution for ${xrayComponent} test suite ran on ${currentDate} at ${currentTime}`,
        description:
