...
class ToolkitWrapper {
  ...
  async deployMintSendUnshielded(opts: MinterFlowOptions): Promise<MinterFlowResult> {
    ...
    await this.execToolkit(
      [
        TOOLKIT_BIN, 'generate-intent', 'deploy', '-c', configFile,
        '--toolkit-js-path', toolkitJsPath, '--coin-public', coinPublic,
        '--output-intent', out('deploy.bin'),
        '--output-private-state', out('initial_state.json'),
        '--output-zswap-state', out('deploy_zswap.json'),
      ],
      'minter generate-intent deploy failed',
    );
    await this.execToolkit(
      [
        TOOLKIT_BIN, 'send-intent', '--intent-file', out('deploy.bin'),
        '--compiled-contract-dir', compiledContractDir, '--dest-file', out('deploy.mn'),
      ],
    ...
    await this.execToolkit(
      [TOOLKIT_BIN, 'contract-state', '--contract-address', contractAddress, '--dest-file', out('state.mn')],
      'minter contract-state failed',
    );
    const tokenType = (
      await this.execToolkit(
        [
          TOOLKIT_BIN, 'show-token-type', '--contract-address', contractAddress,
          '--domain-sep', opts.domainSep, '--unshielded',
        ],
    ...
    await this.execToolkit(
      [
        TOOLKIT_BIN, 'generate-intent', 'circuit', '-c', configFile,
        '--toolkit-js-path', toolkitJsPath, '--coin-public', coinPublic,
        '--input-onchain-state', out('state.mn'),
        '--input-private-state', out('initial_state.json'),
        '--contract-address', contractAddress,
        '--output-intent', out('mint_unshielded.bin'),
        '--output-onchain-state', out('state_after_mint.mn'),
        '--output-private-state', out('private_after_mint.json'),
        '--output-zswap-state', out('mint_zswap.json'),
        'mintUnshieldedToSelfTest', opts.domainSep, String(opts.mintAmount),
      ],
    ...
    await this.execToolkit(
      [
        TOOLKIT_BIN, 'generate-intent', 'circuit', '-c', configFile,
        '--toolkit-js-path', toolkitJsPath, '--coin-public', coinPublic,
        '--input-onchain-state', out('state_after_mint.mn'),
        '--input-private-state', out('private_after_mint.json'),
        '--contract-address', contractAddress,
        '--output-intent', out('send_unshielded.bin'),
        '--output-onchain-state', out('state_after_send.mn'),
        '--output-private-state', out('private_after_send.json'),
        '--output-zswap-state', out('send_zswap.json'),
        'sendUnshieldedToUser', tokenType, userAddress, String(opts.sendAmount),
      ],
    ...
    const submit = await this.execToolkit(
      [
        TOOLKIT_BIN, 'send-intent',
        '--intent-file', out('mint_unshielded.bin'),
        '--intent-file', out('send_unshielded.bin'),
        '--compiled-contract-dir', compiledContractDir,
        '--dest-url', rpcUrl,
      ],
